...
const wordArray1 = ['Freelancer', 'Web Developer','Learner', 'Student'];
...
filterButton.forEach(filterProject=>{
  ...
  filterProject.onclick=()=>{
    if(dataId !== 'All'){
      document.querySelector('.projects .carousel').classList.add('noFlex');
      ...
      projectCarousel.classList.remove('noFlex');
      projectCards.forEach(pCards=>{
        pCards.style.display = "block";
      });
    }
...
function scrollCarousel(){
  if ((projectCarousel.scrollLeft + projectCarousel.clientWidth >= projectCarousel.scrollWidth-10)) {
    projectCarousel.style.scrollBehavior = "auto";
    projectCarousel.scrollLeft = 0;
  }else{
    projectCarousel.style.scrollBehavior = "smooth";
    projectCarousel.scrollLeft += projectCards[0].clientWidth;
  }
}
let carouselInterval = setInterval(scrollCarousel,2000);
projectCarousel.onmouseover=()=>{
  clearInterval(carouselInterval);
}
projectCarousel.onmouseout=()=>{
  carouselInterval = setInterval(scrollCarousel,2000);
}
...
if(userLocation){
  document.querySelector('.country-container span div').textContent = userLocation[0];
}
